...
@section('page-description', 'Manage loan portfolio and applications')
...
<div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-green-600 mt-1">{{ number_format($stats['new_this_month']) }} this month</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-blue-600 mt-1">{{ number_format($stats['pending_loans']) }} pending</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">${{ number_format($stats['total_portfolio'], 0) }}</p>
                    <p class="text-xs text-yellow-600 mt-1">${{ number_format($stats['outstanding_balance'], 0) }} outstanding</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['completed_loans']) }}</p>
                    <p class="text-xs text-red-600 mt-1">{{ number_format($stats['defaulted_loans']) }} defaulted</p>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-200">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-3">
                <form method="GET" action="{{ route('loans.index') }}" class="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-3">
                    ...
                    <select name="loan_type" class="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent">
                        ...
                        <option value="auto" {{ request('loan_type') == 'auto' ? 'selected' : '' }}>Auto</option>
                        <option value="education" {{ request('loan_type') == 'education' ? 'selected' : '' }}>Education</option>
                        <option value="home_equity" {{ request('loan_type') == 'home_equity' ? 'selected' : '' }}>Home Equity</option>
                    </select>
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('loans.applications') }}" class="inline-flex items-center px-4 py-2 bg-blue-500 border border-transparent rounded-lg font-medium text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Amount & Terms</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="text-sm text-gray-500">{{ $loan->interest_rate }}% over {{ $loan->term_months }} months</div>
                            <div class="text-sm text-gray-500">${{ number_format($loan->monthly_payment, 0) }}/month</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @switch($loan->status)
                                @case('pending')
                                ...
                                @case('rejected')
                                    ...
                                    @break
                            @endswitch
                            @if($loan->outstanding_balance > 0 && in_array($loan->status, ['active', 'disbursed']))
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ route('loans.show', $loan) }}" class="text-gray-600 hover:text-gray-900" title="View">
                                ...
                                @if(in_array($loan->status, ['pending', 'approved']))
                                    <a href="{{ route('loans.edit', $loan) }}" class="text-blue-600 hover:text-blue-900" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if($loan->status === 'pending')
                                    <form action="{{ route('loans.approve', $loan) }}" method="POST" class="inline" onsubmit="return confirm('Approve this loan application?')">
                                        @csrf
                                ...
                                @if(!in_array($loan->status, ['disbursed', 'active']))
                                    <form action="{{ route('loans.destroy', $loan) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this loan?')" class="inline">
                                        @csrf
                                        @method('DELETE')
    ...
    <div class="bg-white rounded-xl shadow-sm p-12 border border-gray-200 text-center">
        ...
        <p class="text-gray-500 mb-6">Get started by creating your first loan application.</p>
        <a href="{{ route('loans.create') }}" class="inline-flex items-center px-4 py-2 bg-yellow-500 border border-transparent rounded-lg font-medium text-black hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 transition-colors duration-200">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
            </svg>
